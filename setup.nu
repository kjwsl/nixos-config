...
def detect-os []: nothing -> string {
  ...
  let os_version = sys host | get long_os_version?
  if $os_version == null {
    return ""
  }
  let os_info = $os_version | split column ' ' --number 2 | rename os version

  let os = $os_info | get os? | str downcase
  if $os == null {
    return ""
  }

  if ($os | str contains "mac") {
    "darwin"
  } else if ($os | str contains "nixos") {
    "nixos"
  } else if ($os | str contains "android" or $os | str contains "linux") {
    # Fallback: check ANDROID_ROOT for cases where sys host reports "linux"
    if ($env.ANDROID_ROOT? | is-not-empty) { "android" } else { "linux" }
  } else {
    ""
  }
